...
namespace RazorSharp.Memory
{
	...
	public static unsafe class Memory
	{
		...
		public static string SafeToString<T>(IntPtr ptr, int elemOfs = 0)
		{
			...
			if (Assertion.Throws<NullReferenceException>(delegate { s = CSUnsafe.Read<T>(addr.ToPointer()).ToString(); })) {
				return "(null)";
			}

			if (Assertion.Throws<AccessViolationException>(delegate { s = CSUnsafe.Read<T>(addr.ToPointer()).ToString();  })) {
				return "(ave)";
			}
