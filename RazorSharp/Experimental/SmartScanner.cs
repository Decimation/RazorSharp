...
namespace RazorSharp.Experimental
{
	...
	internal unsafe class SmartScanner : IDisposable
	{
		...
		{
			foreach (MemoryBasicInformation v in GetRegions())
		...
		{
			int                        cnt  = 0;
			Dictionary<IntPtr, byte[]> list = new Dictionary<IntPtr, byte[]>();
			foreach (KeyValuePair<IntPtr, byte[]> v in m_regions) // Read aligned
				// Scan for heap memory
				for (int i = 0; i < v.Value.Length; i += memory.Length) {
					ArraySegment<byte> segment = new ArraySegment<byte>(v.Value, i, memory.Length);
		...
		public List<T> ReadAll<T>() where T : class
		{
			...
			foreach (KeyValuePair<IntPtr, byte[]> v in m_regions) {
				...
				for (int i = 0; i < v.Value.Length; i += typeSize) {
					...
					for (int j = i; j < i + typeSize; j++) mem[j - i] = v.Value[j];

					bool valid = IsMethodTable<T>(mem);
					if (valid) {
						validC++;
					}
				}
		...
		{
			List<MemoryBasicInformation> ls      = new List<MemoryBasicInformation>();
